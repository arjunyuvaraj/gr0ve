...
class StoryLoader {
  ...
  Future<List<Scene>> _buildScenesFromJson(
    ...
    for (final sceneId in sceneIds) {
      ...
      final sceneData = sceneDefinitions[sceneId] as Map<String, dynamic>;
  ...
  Future<Scene> _buildSceneFromJson(
    ...
    if (contentPath != null) {
      ...
      lines.add(StoryMessage(text, kind: MessageKind.narrative));
    }
    ...
    return Scene(
      ...
      choices: choices,
      nextScene: nextScene,
      waitDuration: waitSeconds != null ? Duration(seconds: waitSeconds) : null,
    );
  }
    ...
    try {
      ...
      for (final entry in episodes.entries) {
        ...
        for (final sceneId in sceneIds) {
          if (!sceneDefinitions.containsKey(sceneId)) {
            errors.add(
              'Episode $episodeId references undefined scene $sceneId',
            );
          }
        }
      }
    ...
    if (choices != null) {
      for (final choiceData in choices) {
        if (choiceData is Map<String, dynamic>) {
          ...
          if (nextSceneFromChoice != null &&
              !visited.contains(nextSceneFromChoice)) {
            _checkCircularRef(
              nextSceneFromChoice,
              sceneDefinitions,
              visited,
